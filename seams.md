...
| `home` | The home on disk: layout, validation, health, scaffolding, change hints. **The only thing that knows a path.** | `openHome(path) → Home \| HomeProblem`; read · check · scaffold · watch | no |
...
| `work` | Durable work: triggers, due times, leases, claims, retry, jobs. **Decides when and what runs.** | claim · complete · fail · nextDue | **yes** |
...
loops is justified. **Mark the result provisional.**
