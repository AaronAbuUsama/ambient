...
What "closed" means for an area. Every row is a command you can run or a state you can
look at — nothing aspirational, nothing that depends on how it felt. An area is closed
when all nine pass, in order.

Run from the repository root. The skill [`close-area`](../../.claude/skills/close-area/SKILL.md)
runs this list and reports it row by row.

| # | Check | Passes when |
|---|---|---|
| 1 | `vp test` | Every assertion in the area's gate passes, and the gate is the spec's §4 made executable — for SKELETON, the eighteen numbered `it`s in `src/modules/home/home.test.ts`. |
...
| 7 | `docs/design/roadmap.md` status board | The area's row reads `● closed`, and **You are here** names the next area. |
| 8 | `docs/design/roadmap.md` ledger | A dated entry, newest first, two lines: what shipped and what was learned. The area's **Active** detail is deleted in the same commit — the ledger line is what survives. |
...
lines, 1.7% — were extracted into `section` and `open`. **A closing area must not exceed
0 lines.**

Raising the baseline is a decision, not a side effect: if an area genuinely needs to
