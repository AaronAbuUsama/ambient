...
| 3 | ~~**Addressed over MCP, never by path.**~~ **Superseded by [ADR 007](../../adr/007-knowledge-is-files-not-a-client.md):** the base is files, and `knowledge` reads and writes them through a `Place` from `home`. Addressing it over MCP is *what spawns `ok`* — measured, not argued. What survives is the half that always mattered: **no code here spawns `ok`.** | [knowledge.md](../../rules/knowledge.md) § Amendments 1 |
| 4 | ~~**`home` already enforces 3 in the type** — no knowledge grant at all.~~ **Overtaken by ticket 01**, which is what ADR 007 required: `home` now exposes `knowledge` and `index` as `Place` properties, beside `blobs`. `Parsed` is still a closed union of six filenames, so nothing under `knowledge/` is namable as config — the enforcement that mattered is intact; the one it was cited for is gone on purpose. | [`home/types.ts`](../../../src/modules/home/types.ts), [ticket 01](./issues/01-the-floor.md) |
...
| 8 | **`knowledge` already has a [seams.md](../../design/seams.md) row**, so `new-module` will not refuse it. The row no longer reads *"hides the OK MCP client"* — [ADR 007](../../adr/007-knowledge-is-files-not-a-client.md) amended it to **"hides the layout and the codec"**. | [seams.md](../../design/seams.md) |
